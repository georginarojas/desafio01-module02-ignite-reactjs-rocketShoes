...
describe("useCart Hook", () => {
  ...
  it("should be able to initialize cart with localStorage value", () => {
    ...
    expect(result.current.cart).toEqual(initialStoragedData);
  ...
  it("should be able to add a new product", async () => {
    ...
    //// Other solution 2
    // await act(async () => {
    //   await result.current.addProduct(productId);
    // });
    ...
    // expect(result.current.cart).toEqual(expect.arrayContaining(newCart));
  ...
  it("should not be able to update a product amount to a value smaller than 1", async () => {
    ...
    const { result, waitForValueToChange, waitFor } = renderHook(useCart, {
      wrapper: CartProvider,
    });
